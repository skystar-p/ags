...
export function pspec(name: string, type: PspecType = "jsobject", handle: PspecFlag = "r") {
  ...
  switch (type) {
    ...
    default:
      return GObject.ParamSpec.jsobject(
        name,
        name,
        name,
        flags,
...
>(object: Obj, config?: Config) {
  ...
  if (config && config.signals) {
    Object.keys(config.signals).forEach(signal =>
      Signals[signal] = {
  ...
  if (config && config.properties) {
    Object.keys(config.properties).forEach(prop => Properties[prop] = pspec(prop, ...config.properties![prop]));
  }
