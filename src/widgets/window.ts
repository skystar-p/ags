...
export class Window<Child extends Gtk.Widget, Attr> extends Gtk.Window {
  ...
  set anchor(anchor: Anchor[]) {
    if (
      this.anchor.length === anchor.length
  ...
  get margins() {
    return ["TOP", "RIGHT", "BOTTOM", "LEFT"].map(edge => LayerShell.get_margin(this, LayerShell.Edge[edge]));
  }
  ...
  set margins(margin: number[]) {
    ...
    margins.forEach(([side, i]) => LayerShell.set_margin(this, LayerShell.Edge[side], (margin as number[])[i]));
